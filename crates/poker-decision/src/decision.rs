...
use poker_core::{ActionType, GameState, PokerError, Deck};
...
impl DecisionEngine {
    ...
    pub fn analyze(&self, state: &GameState) -> Result<DecisionResult, PokerError> {
        ...
        let hero_cards = state.hero_cards.clone();
        ...
        let hero_array = if hero_cards.len() >= 2 {
            ...
            return Err(PokerError::InvalidGameState { reason: "Hero must have 2 cards" });
        };
        ...
        let mut action_evs: Vec<ActionEV> = candidates.into_iter().map(|c| {
        ...
        if estimated_equity > 0.6 {
            explanation.add_factor(DecisionFactor::StrongEquity, format!("Hand has strong equity ({:.1}%)", estimated_equity * 100.0));
        } else if estimated_equity < 0.4 {
            explanation.add_factor(DecisionFactor::WeakEquity, format!("Hand has weak equity ({:.1}%)", estimated_equity * 100.0));
        }

        if best_action.ev > 0.0 && required_equity > 0.0 && estimated_equity > required_equity {
             explanation.add_factor(DecisionFactor::PotOdds, format!("Good pot odds for call: required {:.1}%, actual {:.1}%", required_equity * 100.0, estimated_equity * 100.0));
        }

        let rec_size = match best_action.action {
            ActionType::Bet(amount) | ActionType::Raise(amount) | ActionType::AllIn(amount) => Some(amount),
            _ => None,
        };
